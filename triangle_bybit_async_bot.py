...
async def simulate_trading_execution(route_id, profit, markets, symbols):
    try:
        # 🔒 Повторная проверка стаканов перед сделками (фронтраннинг-защита)
        # Для примера проверим первый шаг ещё раз
        print(f"[⏳] Повторная проверка стакана перед симуляцией: {route_id}")

        base, mid1, mid2, _ = route_id.split("->") + [None]  # Защита от ValueError
        s1 = f"{mid1}/{base}" if f"{mid1}/{base}" in symbols else f"{base}/{mid1}"
        market_info = markets.get(s1, {})

        # ✅ Контроль округления
        price_precision = market_info.get("precision", {}).get("price", 6)
        qty_precision = market_info.get("precision", {}).get("amount", 6)
        min_qty = market_info.get("limits", {}).get("amount", {}).get("min", 0.0)

        # 🔢 Пример расчета округлённого количества (эмуляция)
        fake_price = 1.23456789
        qty = round(target_volume_usdt / fake_price, qty_precision)

        if qty < min_qty:
            msg = f"❌ Симуляция отклонена: qty={qty} < minQty={min_qty} по {s1}"
            await send_telegram_message(msg)
            return False

        await asyncio.sleep(1)  # имитация задержки при исполнении
