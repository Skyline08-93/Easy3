...
def format_line(index, pair, price, side, volume_usd, color):
    emoji = {"green": "🟢", "yellow": "🟡", "red": "🟥"}.get(color, "")
    return f"{emoji} {index}. {pair} - {price:.6f} ({side}), ${volume_usd:.2f}"
...
async def check_triangle(base, mid1, mid2, symbols):
    try:
        ...
        price1, vol1 = await get_execution_price(s1, "buy" if f"{mid1}/{base}" in symbols else "sell", target_volume_usdt)
        ...
        price2, vol2 = await get_execution_price(s2, "buy" if f"{mid2}/{mid1}" in symbols else "sell", target_volume_usdt)
        ...
        price3, vol3 = await get_execution_price(s3, "sell" if f"{mid2}/{base}" in symbols else "buy", target_volume_usdt)
        ...
        min_liquidity = round(min(vol1, vol2, vol3), 2)
        pure_profit_usdt = round((result - 1) * target_volume_usdt, 2)

        message = "\n".join([
            format_line(1, s1, price1, side1, vol1, "green"),
            format_line(2, s2, price2, side2, vol2, "yellow"),
            format_line(3, s3, price3, side3, vol3, "red"),
            "",
