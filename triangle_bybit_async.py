...
exchange = ccxt.bybit()
symbol_rules = {}  # Глобальный словарь правил округления
...
async def load_symbols():
    ...
    for symbol, data in markets.items():
        if data.get('precision') and data.get('limits'):
            tick_size = data['precision'].get('price', None)
            min_price = data['limits']['price'].get('min', None) if data.get('limits') and data['limits'].get('price') else None
            symbol_rules[symbol] = {
                'tick_size': tick_size,
                'min_price': min_price
            }
...
def round_price(symbol, price):
    ...
    if not rule or rule['tick_size'] is None:
        return price
    tick_size = rule['tick_size']
    precision = int(abs(round(-1 * (tick_size ** 0.5))) + 1) if tick_size < 1 else 0
